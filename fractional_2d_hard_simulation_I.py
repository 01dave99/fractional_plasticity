...
for alpha in alphas:
	for I in Is:
		...
		if len(cells)>0:
			...
			with open("results/frac_defl_I_"+str(I)+"_alpha_"+str(alpha)+"_"+str(num_dofs_global)+".txt", 'w') as wfile:
				wfile.write("\n"+str(ydefl))	

		if msh.comm.rank==0:
			np.savetxt("results/frac_res_I_"+str(I)+"_alpha_"+str(alpha)+"_"+str(num_dofs_global)+".csv",residuals,delimiter=",")
