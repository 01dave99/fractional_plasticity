...
for I in Is:
	...
	if msh.comm.rank==0:
		np.savetxt("frac_res_"+str(num_dofs_global)+"_I"+str(I_count)+".csv",residuals,delimiter=",")
		np.savetxt("defl_mid_"+str(num_dofs_global)+"_I"+str(I_count)+".csv",defl_mid,delimiter=",")
		np.savetxt("defl_right_"+str(num_dofs_global)+"_I"+str(I_count)+".csv",defl_right,delimiter=",")

	with io.XDMFFile(msh.comm, "frac_uh_final_"+str(num_dofs_global)+"_I"+str(I_count)+".xdmf", "w") as file:
				file.write_mesh(msh)
				file.write_function(uh)