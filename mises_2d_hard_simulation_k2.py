...
for k2 in k2s:
		...
		if ti==0:
			for m in range(100):
				if m==0:
					...
					if m==99:
						if msh.comm.rank==0:
							print("Did not converge in 100 steps!",flush=True)
	...
	if len(cells)>0:
		...
		with open("results/defl_"+str(num_dofs_global)+"_k2_"+str(k2)+".txt", 'w') as wfile:
			wfile.write("\n"+str(ydefl))	

	if msh.comm.rank==0:
		np.savetxt("results/res_"+str(num_dofs_global)+"_k2_"+str(k2)+".csv",residuals,delimiter=",")
