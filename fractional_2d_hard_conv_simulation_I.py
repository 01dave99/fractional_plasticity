...
k1=10000
k2=60000
alphas=[0.4,0.5,0.7,0.9,0.99,0.999]
Is=[1,100]
tmax=15000
steps=200
eps=math.pow(10,-8)
...
for alpha in alphas:
	for I in Is:
		...
		def eval_r(sigma,chi1,chi2):
			...
			r1=I**(1-alpha)*(df(sigma,chi1,chi2)+(1-alpha)*df1)
			r2=I**(1-alpha)*(df(sigma,chi1,chi2)+(1-alpha)*df2)
			return(0.5*(r1+r2))
		...
		if len(cells)>0:
			...
			with open("results/frac_conv_defl_I_"+str(I)+"_alpha_"+str(alpha)+"_"+str(num_dofs_global)+".txt", 'w') as wfile:
				wfile.write("\n"+str(ydefl))	

		if msh.comm.rank==0:
			np.savetxt("results/frac_conv_res_I_"+str(I)+"_alpha_"+str(alpha)+"_"+str(num_dofs_global)+".csv",residuals,delimiter=",")
