...
h=[d1/n d4/n d5/n; d4/n d2/n d6/n ;d5/n d6/n d3/n];


for j=1:m-1
    sig1=-2/sqrt(3)*y0+(j*4/sqrt(3)*y0/m);
    sigma=[ sig1 0 0; 0 (sig1+sqrt(4*y0^2-3*sig1^2))/2 0; 0 0 0];
...
m=0:100;
X1=-2/sqrt(3)*y0+(m*4/sqrt(3)*y0/(length(m)-1));
Y1=(X1-sqrt(4*y0^2-3*X1.^2))/2;
Y2=(X1+sqrt(4*y0^2-3*X1.^2))/2;
plot([X1 fliplr(X1)],[Y1 fliplr(Y2)])
