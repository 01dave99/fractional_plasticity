...
for k1 in k1s:
	...
	if len(cells)>0:
		...
		with open("results/frac_defl_"+str(num_dofs_global)+"_k1_"+str(k1)+".txt", 'w') as wfile:
			wfile.write("\n"+str(ydefl))	

	if msh.comm.rank==0:
		np.savetxt("results/frac_res_"+str(num_dofs_global)+"_k1_"+str(k1)+".csv",residuals,delimiter=",")
