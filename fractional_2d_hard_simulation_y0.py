...
for y0 in y0s:
	...
	if len(cells)>0:
		...
		with open("results/frac_defl_"+str(num_dofs_global)+"_y0_"+str(y0)+".txt", 'w') as wfile:
			wfile.write("\n"+str(ydefl))	

	if msh.comm.rank==0:
		np.savetxt("results/frac_res_"+str(num_dofs_global)+"_y0_"+str(y0)+".csv",residuals,delimiter=",")
